...
public class CafeOrderChecker {
    ...
    public static boolean isFirstComeFirstServed(int[] takeOutOrders, int[] dineInOrders, int[] servedOrders) {
        ...
        int takeOutOrdersCount = 0;
        int dineInOrdersCount = 0;

        for(int servedOrder : servedOrders){
            if(takeOutOrdersCount < takeOutOrders.length && servedOrder == takeOutOrders[takeOutOrdersCount]){

                takeOutOrdersCount++;

            } else if(dineInOrdersCount < dineInOrders.length && servedOrder == dineInOrders[dineInOrdersCount]){
        ...
        if(dineInOrdersCount != dineInOrders.length - 1 && takeOutOrdersCount != takeOutOrders.length){
            return false;
        }
